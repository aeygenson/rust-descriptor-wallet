...
/// Strongly-typed descriptor/address derivation index.
///
/// This is useful for receive-address management, address history,
/// descriptor inspection, and UI/API flows that should not pass raw
/// derivation indexes around as plain integers.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Default)]
pub struct AddressIndex(pub u32);

impl AddressIndex {
    /// Create an address-index wrapper.
    pub fn new(value: u32) -> Self {
        Self(value)
    }

    /// Access the inner derivation index.
    pub fn as_u32(self) -> u32 {
        self.0
    }
}
...
impl fmt::Display for WalletKeychain {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.as_str())
    }
}

/// Address purpose relative to wallet usage.
///
/// This is intentionally separate from `WalletKeychain`: keychain describes
/// derivation branch, while usage describes how the wallet/UI currently treats
/// an address.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum WalletAddressUsage {
    /// Address has been generated but no wallet transaction currently spends to it.
    #[default]
    Unused,
    /// Address appears in wallet history or has received funds.
    Used,
    /// Address belongs to change/internal wallet activity.
    Change,
}

impl WalletAddressUsage {
    pub fn as_str(self) -> &'static str {
        match self {
            WalletAddressUsage::Unused => "unused",
            WalletAddressUsage::Used => "used",
            WalletAddressUsage::Change => "change",
        }
    }
}

impl fmt::Display for WalletAddressUsage {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.as_str())
    }
}

/// Source of an address record exposed to UI/API layers.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum WalletAddressSource {
    /// Address was generated by the wallet receive flow.
    #[default]
    Generated,
    /// Address was discovered by scanning wallet history or descriptors.
    Discovered,
    /// Address metadata was imported from an external source.
    Imported,
}

impl WalletAddressSource {
    pub fn as_str(self) -> &'static str {
        match self {
            WalletAddressSource::Generated => "generated",
            WalletAddressSource::Discovered => "discovered",
            WalletAddressSource::Imported => "imported",
        }
    }
}

impl fmt::Display for WalletAddressSource {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.as_str())
    }
}

/// User-facing address label category.
///
/// This is deliberately small for now. Free-form label text should live in
/// a higher-level model/DTO, while this enum captures stable semantic category.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Default)]
pub enum WalletAddressLabelKind {
    /// No semantic label category has been assigned.
    #[default]
    None,
    /// Address is associated with an invoice/payment request.
    Invoice,
    /// Address is associated with a counterparty/contact.
    Contact,
    /// Address is used for an internal note or custom purpose.
    Custom,
}

impl WalletAddressLabelKind {
    pub fn as_str(self) -> &'static str {
        match self {
            WalletAddressLabelKind::None => "none",
            WalletAddressLabelKind::Invoice => "invoice",
            WalletAddressLabelKind::Contact => "contact",
            WalletAddressLabelKind::Custom => "custom",
        }
    }
}

impl fmt::Display for WalletAddressLabelKind {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", self.as_str())
    }
}
